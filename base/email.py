...
from .models import Submissions,Assignments,Classrooms,Students,CustomUser,Materials
...
def material_post_mail(classroom_id,material_id):
    users = Students.objects.filter(classroom_id = classroom_id)
    email_list = [user.student_id.email for user in users]
    ...
    message = 'Dear Students, {} assignment has been posted to {}. Due date of the assignment is {}'.format(material_name,classroom_name,due_date)
    instructions = 'Instructions of the assignment are: {}'.format(material.instructions)
    message = message + '\n' + instructions
    subject = 'New Assignment in {} class'.format(classroom_name)
    send_email(subject,email_list,message)
